...
% inputs
[windowLength, overlap, useHanningWindow, useFilter, takeLogs, verbose,downsample,absolute,saveMemory] = ...
...
HE = hilbert_envelope(verbose,absolute,saveMemory);
clear signal
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function HE = hilbert_envelope(verbose,absolute,saveMemory)
...
if saveMemory
    if absolute
        ...
        for iVox = ROInets.rows(signal):-1:1,
            HE(iVox,:) = transpose(hilbert(transpose(signal(iVox,:)), nfft));
            % free up memory
            signal(iVox,:) = [];
        end%for
    end
else
    HE=transpose(hilbert(transpose(signal), nfft));
    if isa(HE,'gpuArray')
        HE=gather(HE);
    end
end
...
P.addParamValue('absolute',         true,                @islogical);

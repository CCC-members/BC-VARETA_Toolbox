    ...
    if(analysis_method.(method_name).run)
        ...
        if(properties.BC_V_info.properties.general_params.run_by_trial.value)
            if(properties.BC_V_info.properties.general_params.run_frequency_bin.value)
                ...
                connectivity_level.(method_name).(band.name).(f_bin).name       = properties.file_name;
                connectivity_level.(method_name).(band.name).(f_bin).ref_path   = reference_path{2};
                properties.BC_V_info.(trial_name).connectivity_level            = connectivity_level;                
            else
                connectivity_level.(method_name).(band.name).name               = properties.file_name;
                connectivity_level.(method_name).(band.name).ref_path           = reference_path{2};
                properties.BC_V_info.(trial_name).connectivity_level            = connectivity_level;
            end
        else
            if(properties.BC_V_info.properties.general_params.run_frequency_bin.value)
                ...
                connectivity_level.(method_name).(band.name).(f_bin).name       = properties.file_name;
                connectivity_level.(method_name).(band.name).(f_bin).ref_path   = reference_path{2};
                properties.BC_V_info.connectivity_level = connectivity_level;
            else
                connectivity_level.(method_name).(band.name).name               = properties.file_name;
                connectivity_level.(method_name).(band.name).ref_path           = reference_path{2};
                properties.BC_V_info.connectivity_level                         = connectivity_level;

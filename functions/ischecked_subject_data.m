...
if(isstruct(subject_info.leadfield_dir))
    for i=1:length(subject_info.leadfield_dir)
        ...
        if(~isfile(fullfile(root_dir,subject_info.leadfield_dir(i).path)))
...
if(isempty(error_msg_array))
    ...
    if(isequal(sources,'default'))
        ...
        if(isempty(leadfield))
            ...
            error_msg2 = strcat("----------  OR  -------------");  
            error_msg3 = strcat("The slected method: ", method , " is not correct."); 
            error_msg_array = [error_msg_array; error_msg; error_msg2; error_msg3];
            return;
        end        
    else
        if(ischar(sources))

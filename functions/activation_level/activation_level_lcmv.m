...
method              = activation_params.methods{3};
...
%%
if IsCurv == 0    
    [s2j,sigma_post,T,~,~,scaleSvv,scaleLvj] = lcmv(Svv,Lvj,param);
    clearvars param Svv;
...
elseif IsCurv == 1
    ...
    [s2j_giri,sigma_post_giri,Tgiri,~,~,scaleSvv_giri,scaleLvj_giri] = lcmv(Svv,subject.Lvj_giri,param);
    param.flag = strcat(flag," Sulci compensation");
    [s2j_sulc,sigma_post_sulc,Tsulc,~,~,scaleSvv_sulc,scaleLvj_sulc] = lcmv(Svv,subject.Lvj_sulc,param);
    clearvars param Svv;
